...
# def eval_max_score(scores, items, tars, ks, his_seqs, full_base_scores=None, pmi_dict=None):
#     # def eval_max_score(scores, tars, ks):
#     '''
#     recall according to max score rule
#     Args:
...
#         tars: batch_size, each element is a list containing ground-truth items
#         k: top k
#         pmi_dict: pmi_dict[a][b] 表示 a 和 b 之间的 PMI 值
#     Returns:
#     '''
#     items = list(map(lambda x: x + 1, items))  # 编号+1
#     batch_recall = [0] * len(ks)
...
#
#     for sc, it, tar, seq in zip(scores, items, tars, his_seqs):
#         n_interest, n_recall = it.shape
#         in_nbr = (np.arange(n_interest) + 1).repeat(n_recall)
#         item_list = list(zip(in_nbr.reshape(-1), it.reshape(-1), sc.reshape(-1)))
#         item_list.sort(key=lambda x: x[2], reverse=True)
#         his_item_set = set(list(seq))
#         for nbr_k, k in enumerate(ks):
#             rec_its = [item for item in item_list[:k]if item[1] not in his_item_set]
#             tmp_idx = k
#             while len(rec_its) < k and tmp_idx < len(item_list):
#                 if item_list[tmp_idx][1] not in his_item_set:
#                     rec_its.append(item_list[tmp_idx])
#                 tmp_idx += 1
#             if k == ks[-1]: rec_results.append(rec_its)
#             rec_its = set(list(map(lambda x: x[1], rec_its))).intersection(set(tar))
#             batch_recall[nbr_k] += len(rec_its) / len(tar)
#             if len(rec_its):
#                 batch_hit[nbr_k] += 1
#     # '''
#     return batch_recall, batch_hit, rec_results
